...
test("Просмотр логов (test 18)", async ({ page }) => {
  ...
  await expect(page.locator("(//span[text()='Журнал подключений'])[2]")).toBeVisible()
  ...
  await expect(page.locator("(//span[text()='Лог'])[1]")).toBeVisible()
  await expect(page.locator("(//span[text()='Все поля'])[1]")).toBeVisible()
  ...
  //- Поле
  await expect(page.locator("(//div[text()='Поле'])[1]")).toBeVisible()
  //- Новое значение
  await expect(page.locator("(//div[text()='Новое значение'])[1]")).toBeVisible()
  //- Старое значение.
  await expect(page.locator("(//div[text()='Старое значение'])[1]")).toBeVisible()
  ...
  await expect(page.locator("(//span[@class='block log-new'])[1]")).toContainText(/[0-9]/)
  ...
  await expect(page.locator("(//span[@class='log-date'])[1]")).toContainText("17.01.2023")
  ...
  await expect(page.locator("(//div[@class='fc_table_results']//div)[1]")).toContainText("SHETININM")
