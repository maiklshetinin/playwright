...
const INN_ARR = [
  '0',               //3.1. Все нули (1 символ)
  '000000000000000', //3.2. Все нули (15 символов)
  '1234567890',      //3.3. произвольные цифры (10 символов)
  '123456789012',    //3.4. Произвольные цифры (12 символов)
  '123456789012345', //3.5. Произвольные цифры (15 символов)
  '*?:;№:"!%:???:?', //3.6. Специальные символы (*?:;№:"!%:???:?)
  'йцукенгшрпамыпа', //3.7. Произвольный текст (йцукенгшрпамыпа)
]
...
test("ЮЛ Контроль при вводе значений ИНН (test 25)", async ({ page }) => {
  const CASHE_Page = new CASHE(page)
  await CASHE_Page.login(LOGIN, PASSWORD)
  await page.waitForLoadState("networkidle")
  ...
  //3. В поле ИНН ввести одно из значений: 7892088124 / 6565554542 / 7714636130 / 7725514969
  await page.locator(Card.owner).scrollIntoViewIfNeeded()
  await CASHE_Page.click(Card.owner)
...
test("ЮЛ Контроль при вводе значений ИНН  (валидация) (test 25.1)", async ({ page }) => {
  const CASHE_Page = new CASHE(page)
  await CASHE_Page.login(LOGIN, PASSWORD)
  await page.waitForLoadState("networkidle")
